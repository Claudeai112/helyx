export const heroVertexShader = /* glsl */ `
  ...
  void main() {
    ...
    float displace = n * 0.38 + n2 * 0.12 + mouseInfl;
...
export const heroFragmentShader = /* glsl */ `
  ...
  void main() {
    vec3 purple = vec3(0.423, 0.361, 0.906); // #6c5ce7
    ...
    vec3 col = mix(purple, violet, t);
    col = mix(col, cyan, smoothstep(0.3, 0.9, t2));

    float fres = pow(1.0 - max(dot(normalize(vNormal), vec3(0.0, 0.0, 1.0)), 0.0), 2.0);
    col += fres * vec3(0.35, 0.25, 0.55);

    // subtle vignette darkening at the back
    float depth = smoothstep(-2.0, 2.0, vPos.z);
    col *= 0.6 + depth * 0.4;

    // mouse-reactive bloom tint
    float m = length(uMouse) * 0.25;
    col += vec3(m * 0.12, m * 0.06, m * 0.15);
